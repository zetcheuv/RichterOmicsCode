...
EPIC_rgs <- read.metharray(paste0("idats_850k/", finalPHENO850k$Basename), force=T)
FULL_rgs <- combineArrays(BC450k_rgs, EPIC_rgs, outType="IlluminaHumanMethylation450k", force=T) #force=T in case of multiple EPIC chip versions
...
### And now for some statistics / linear modelling - DMRs - Somme functional annotations
...
# [mod] = fit bayesian model - for conveniance
